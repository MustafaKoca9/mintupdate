...
class CacheWatcher(threading.Thread):
    ...
    def run(self):
        ...
        if os.path.isfile(self.pkgcache) and os.path.isfile(self.dpkgstatus):
            ...
            self.application.logger.write(
                "Package cache location not found, disabling cache monitoring"
            )
        ...
        while True:
            if not self.paused and self.application.window.get_sensitive():
                try:
                    ...
                    if (
                        cachetime != self.cachetime or statustime != self.statustime
                    ) and not self.application.dpkg_locked():
    ...
    def refresh_cache(self):
        self.application.logger.write(
            "Changes to the package cache detected; triggering refresh"
        )
...
class ChangelogRetriever(threading.Thread):
    ...
    def get_ppa_info(self):
        ...
        try:
            with open(source) as f:
                for line in f:
                    if not line.startswith("#") and all(
                        word in line for word in ppa_words
                    ):
    ...
    def get_ppa_changelog(self, ppa_owner, ppa_name):
        ...
        print(
            "\nFetching changelog for PPA package %s/%s/%s ..."
            % (ppa_owner, ppa_name, self.source_package)
        )
        ...
        deb_dsc_uri = (
            "https://ppa.launchpadcontent.net/%s/%s/ubuntu/pool/main/%s/%s/%s_%s.dsc"
            % (
                ppa_owner,
                ppa_name,
                ppa_abbr,
        ...
        try:
            deb_dsc = (
                urllib.request.urlopen(deb_dsc_uri, None, 10).read().decode("utf-8")
            )
        except Exception as e:
        ...
        if int(deb_size) > max_tarball_size:
            print(
                "Tarball size %s B exceeds maximum download size %d B. Skipping download."
                % (deb_size, max_tarball_size)
            )
            return
        deb_file_uri = (
            "https://ppa.launchpadcontent.net/%s/%s/ubuntu/pool/main/%s/%s/%s"
            % (ppa_owner, ppa_name, ppa_abbr, self.source_package, deb_filename)
        )
        try:
            deb_file = (
                urllib.request.urlopen(deb_file_uri, None, 10).read().decode("utf-8")
            )
        except Exception as e:
    ...
    def run(self):
        ...
        if self.origin == "linuxmint":
            changelog_sources.append(
                "http://packages.linuxmint.com/dev/"
                + self.source_package
                + "_"
                + self.version
                + "_amd64.changes"
            ...
            if self.is_kernel_update:
                ...
                changelog_sources.append(
                    "https://changelogs.ubuntu.com/changelogs/pool/main/l/linux/linux_%s/changelog"
                    % (kernel_version)
                )
            elif self.source_package.startswith("lib"):
                changelog_sources.append(
                    "https://changelogs.ubuntu.com/changelogs/pool/main/%s/%s/%s_%s/changelog"
                    % (
                        self.source_package[0:4],
                        self.source_package,
                        self.source_package,
                        self.version,
                    )
                ...
                changelog_sources.append(
                    "https://changelogs.ubuntu.com/changelogs/pool/multiverse/%s/%s/%s_%s/changelog"
                    % (
                        self.source_package[0:4],
                        self.source_package,
                        self.source_package,
                ...
                changelog_sources.append(
                    "https://changelogs.ubuntu.com/changelogs/pool/universe/%s/%s/%s_%s/changelog"
                    % (
                        self.source_package[0:4],
                        self.source_package,
                        self.source_package,
                ...
                changelog_sources.append(
                    "https://changelogs.ubuntu.com/changelogs/pool/restricted/%s/%s/%s_%s/changelog"
                    % (
                        self.source_package[0:4],
                        self.source_package,
                        self.source_package,
            ...
            if self.is_kernel_update:
                changelog_sources.append(
                    "https://metadata.ftp-master.debian.org/changelogs/main/l/linux/linux_%s_changelog"
                    % (self.version)
                )
            elif self.source_package.startswith("lib"):
                changelog_sources.append(
                    "https://metadata.ftp-master.debian.org/changelogs/main/%s/%s/%s_%s_changelog"
                    % (
                        self.source_package[0:4],
                        self.source_package,
                        self.source_package,
                ...
                changelog_sources.append(
                    "https://metadata.ftp-master.debian.org/changelogs/contrib/%s/%s/%s_%s_changelog"
                    % (
                        self.source_package[0:4],
                        self.source_package,
                        self.source_package,
                ...
                changelog_sources.append(
                    "https://metadata.ftp-master.debian.org/changelogs/non-free/%s/%s/%s_%s_changelog"
                    % (
                        self.source_package[0:4],
                        self.source_package,
                        self.source_package,
            ...
            if ppa_owner and ppa_name:
                ...
                if not deb_changelog:
                    changelog_sources.append(
                        "https://launchpad.net/~%s/+archive/ubuntu/%s/+files/%s_%s_source.changes"
                        % (ppa_owner, ppa_name, self.source_package, self.version)
                    )
                else:
                    changelog = "%s\n" % deb_changelog
            else:
            ...
            try:
                ...
                if "linuxmint.com" in changelog_source:
                    ...
                    for change in changes:
                        ...
                        if (
                            change == ""
                            or stripped_change.startswith("*")
                            or stripped_change.startswith("[")
                        ):
                            changelog = changelog + change + "\n"
                elif "launchpad.net" in changelog_source:
                    changes = (
                        source.split("Changes:")[1].split("Checksums")[0].split("\n")
                    )
                    for change in changes:
                        stripped_change = change.strip()
                        if stripped_change != "":
...
class AutomaticRefreshThread(threading.Thread):
    ...
    def run(self):
        ...
        while self.application.refresh_schedule_enabled:
            try:
                schedule = {
                    "minutes": self.application.settings.get_int(
                        "%srefresh-minutes" % settings_prefix
                    ),
                    "hours": self.application.settings.get_int(
                        "%srefresh-hours" % settings_prefix
                    ),
                    "days": self.application.settings.get_int(
                        "%srefresh-days" % settings_prefix
                    ),
                }
                timetosleep = (
                ...
                if not timetosleep:
                    ...
                    if not initial_refresh:
                        refresh_last_run = self.application.settings.get_int(
                            "refresh-last-run"
                        )
                    ...
                    schedule["hours"] = int(
                        (timetosleep - schedule["days"] * day) / hour
                    )
                    ...
                    self.application.logger.write(
                        "%s refresh will happen in %d day(s), %d hour(s) and %d minute(s)"
                        % (
                            refresh_type.capitalize(),
                            schedule["days"],
                            schedule["hours"],
                            schedule["minutes"],
                        )
                    ...
                    if not self.application.refresh_schedule_enabled:
                        self.application.logger.write(
                            "Auto-refresh disabled in preferences; cancelling %s refresh"
                            % refresh_type
                        )
                    ...
                    if self.application.app_hidden():
                        self.application.logger.write(
                            "Update Manager is in tray mode; performing %s refresh"
                            % refresh_type
                        )
                        ...
                        if initial_refresh:
                            self.application.logger.write(
                                "Update Manager window is open; skipping %s refresh"
                                % refresh_type
                            )
                        else:
                            self.application.logger.write(
                                "Update Manager window is open; delaying %s refresh by 60s"
                                % refresh_type
                            )
                            time.sleep(60)
            except Exception as e:
                ...
                self.application.logger.write_error(
                    "Exception occurred during %s refresh: %s"
                    % (refresh_type, str(sys.exc_info()[0]))
                )
            ...
            self.application.logger.write(
                "Auto-refresh disabled in preferences, AutomaticRefreshThread stopped"
            )
...
class InstallThread(threading.Thread):
    def __init__(self, application):
        ...
        self.application = application
        self.application.window.get_window().set_cursor(
        ...
        try:
                        ...
                        for pkg in update.package_names:
                            if "-image-" in pkg:
                                try:
                                    ...
                                    print(
                                        "Warning: Could not assess the current kernel version: %s"
                                        % str(e)
                                    )
                                    self.reboot_required = True
                                break
            ...
            if aptInstallNeeded:
                try:
                    ...
                    warnings = subprocess.check_output(
                        "/usr/lib/linuxmint/mintUpdate/checkWarnings.py %s" % pkgs,
                        shell=True,
                    ).decode("utf-8")
                    ...
                    if len(warnings) == 2:
                        ...
                        if len(installations) > 0 or len(removals) > 0:
                            ...
                            try:
                                ...
                                dialog.set_markup(
                                    "<b>"
                                    + _("This upgrade will trigger additional changes")
                                    + "</b>"
                                )
            ...
            if aptInstallNeeded and proceed:
                ...
                self.application.set_status(
                    ...
                    _("Installing updates"),
                    "mintupdate-installing-symbolic",
                    True,
                )
                Gdk.threads_leave()
                ...
                if os.environ.get("XDG_SESSION_TYPE", "x11") == "x11":
                    cmd += [
                        "--parent-window-id",
                        "%s" % self.application.window.get_window().get_xid(),
                    ]
                ...
                if update_successful:
                    ...
                    elif self.application.settings.get_boolean(
                        "hide-window-after-update"
                    ):
                        Gdk.threads_enter()
                    ...
                    if [pkg for pkg in PRIORITY_UPDATES if pkg in packages]:
                        ...
                        self.application.uninhibit_pm()
                        self.application.logger.write(
                            "Mintupdate was updated, restarting it..."
                        )
            ...
            if update_flatpaks and proceed:
                ...
                if self.application.flatpak_updater.error is not None:
                    self.application.set_status_message_from_thread(
                        self.application.flatpak_updater.error
                    )
                needs_refresh = True

            if proceed and len(cinnamon_spices) > 0:
                Gdk.threads_enter()
                spices_install_window = Gtk.Window(
                    title=_("Updating Cinnamon Spices"),
                    default_width=400,
                    default_height=100,
                    deletable=False,
            ...
            print(e)
            self.application.logger.write_error(
                "Exception occurred in the install thread: " + str(sys.exc_info()[0])
            )
            Gdk.threads_enter()
            self.application.set_status(
                _("Could not install the security updates"),
...
class RefreshThread(threading.Thread):
    ...
    def run(self):
        ...
        if self.root_mode:
            while self.application.dpkg_locked():
                self.application.logger.write(
                    "Package management system locked by another process; retrying in 60s"
                )
                time.sleep(60)
        ...
        try:
            if self.root_mode:
                self.application.logger.write(
                    "Starting refresh (retrieving lists of updates from remote servers)"
                )
            else:
            ...
            # Starts the blinking
            self.application.set_status(
                _("Checking for package updates"),
                _("Checking for updates"),
                "mintupdate-checking-symbolic",
            ...
            if self.root_mode:
                ...
                if (not self.application.app_hidden()) and os.environ.get(
            ...
            if CINNAMON_SUPPORT:
                if self.root_mode:
                    self.application.logger.write(
                        "Refreshing available Cinnamon updates from the server"
                    )
                    self.application.set_status_message_from_thread(
                        _("Checking for Cinnamon spices")
                    )
                    for spice_type in cinnamon.updates.SPICE_TYPES:
                        try:
                            self.application.cinnamon_updater.refresh_cache_for_type(
                                spice_type
                            )
                        except:
                            self.application.logger.write_error(
                                "Something went wrong fetching Cinnamon %ss: %s"
                                % (spice_type, str(sys.exc_info()[0]))
                            )
                            print(
                                "-- Exception occurred fetching Cinnamon %ss:\n%s"
                                % (spice_type, traceback.format_exc())
                            )

            if FLATPAK_SUPPORT:
                if self.root_mode:
                    self.application.logger.write(
                        "Refreshing available Flatpak updates"
                    )
                    self.application.set_status_message_from_thread(
                        _("Checking for Flatpak updates")
                    )
            ...
            if os.getenv("MINTUPDATE_TEST") is None:
                output = subprocess.run(
                    "/usr/lib/linuxmint/mintUpdate/checkAPT.py", stdout=subprocess.PIPE
                ).stdout.decode("utf-8")
            else:
                if os.path.exists(
                    "/usr/share/linuxmint/mintupdate/tests/%s.test"
                    % os.getenv("MINTUPDATE_TEST")
                ):
                    output = subprocess.run(
                        "sleep 1; cat /usr/share/linuxmint/mintupdate/tests/%s.test"
                        % os.getenv("MINTUPDATE_TEST"),
                        shell=True,
            ...
            if "CHECK_APT_ERROR" in output:
                ...
                try:
                    ...
                    if (
                        "apt.cache.FetchFailedException" in output
                        and " changed its " in error_msg
                    ):
                        error_msg += "\n\n%s" % _(
                            "Run 'apt update' in a terminal window to address this"
                        )
                except:
                    error_msg = ""
            ...
            if os.getenv("MINTUPDATE_TEST") == "layer-error" or (not mint_layer_found):
                error_found = True
                self.application.logger.write_error(
                    "Error: The APT policy is incorrect!"
                )

                label1 = _("Your APT configuration is corrupt.")
                label2 = _(
                    "Do not install or update anything - doing so could break your operating system!"
                )
                label3 = _(
                ...
                self.application.set_status(
                    _("Could not refresh the list of updates"),
                    "%s\n%s" % (label1, label2),
                    "mintupdate-error-symbolic",
                    True,
                )
            ...
            if error_found:
                ...
                if error_msg:
                    self.application.builder.get_object("label_error_details").set_text(
                        error_msg
                    )
            ...
            if len(lines):
                for line in lines:
                    ...
                    origin = (
                        origin.replace("linuxmint", "Linux Mint")
                    ...
                    model.set_value(
                        iter, UPDATE_SOURCE, "%s / %s" % (origin, update.archive)
                    )
                    ...
                    model.set_value(
                        iter,
                        UPDATE_TYPE_PIX,
                        "mintupdate-type-%s-symbolic" % update.type,
                    )
                    ...
                    model.set_value(
                        iter,
                        UPDATE_SORT_STR,
                        "%s%s" % (str(type_sort_key), update.display_name),
                    )
            ...
            if CINNAMON_SUPPORT and not is_self_update:
                ...
                for update in self.application.cinnamon_updater.get_updates():
                    ...
                    update.source_packages = [
                        "%s=%s" % (update.uuid, update.new_version)
                    ]
                    update.package_names = []
                    update.type = "cinnamon"
                    if (
                        update.uuid in blacklist
                        or update.source_packages[0] in blacklist
                    ):
                        continue
                    ...
                    if self.application.settings.get_boolean("show-descriptions"):
                        model.set_value(
                            ...
                            "<b>%s</b>\n%s"
                            % (
                                GLib.markup_escape_text(update.uuid),
                        ...
                        model.set_value(
                            iter,
                            UPDATE_DISPLAY_NAME,
                            "<b>%s</b>" % GLib.markup_escape_text(update.uuid),
                        )
                    ...
                    model.set_value(
                        iter,
                        UPDATE_SORT_STR,
                        "%s%s" % (str(type_sort_key), update.uuid),
                    )
                    model.set_value(iter, UPDATE_OBJ, update)
                    num_software += 1
                    num_visible += 1
            ...
            if (
                FLATPAK_SUPPORT
                and self.application.flatpak_updater
                and not is_self_update
            ):
                type_sort_key = 5
                blacklist = self.application.settings.get_strv("blacklisted-packages")
                    ...
                    for update in self.application.flatpak_updater.updates:
                        ...
                        if self.application.settings.get_boolean("show-descriptions"):
                            ...
                            model.set_value(
                                iter,
                                UPDATE_DISPLAY_NAME,
                                "<b>%s</b>" % GLib.markup_escape_text(update.name),
                            )
                        ...
                        model.set_value(iter, UPDATE_SIZE, update.size)
                        model.set_value(
                            iter, UPDATE_SIZE_STR, size_to_string(update.size)
                        )
                        model.set_value(
                            iter, UPDATE_TYPE_PIX, "mintupdate-type-flatpak-symbolic"
                        )
                        ...
                        model.set_value(
                            iter,
                            UPDATE_SORT_STR,
                            "%s%s" % (str(type_sort_key), update.ref_name),
                        )
                        model.set_value(iter, UPDATE_OBJ, update)
                        num_software += 1
                        num_visible += 1
            ...
            if tracker.active:
                if tracker.notify():
                    ...
                    security_msg = (
                        gettext.ngettext(
                            "%d security update", "%d security updates", num_security
                        )
                        % num_security
                    )
                    software_msg = (
                        gettext.ngettext(
                            "%d software update", "%d software updates", num_software
                        )
                        % num_software
                    )
                    msg = ""
                    ...
                    self.notification = Notify.Notification.new(
                        notification_title, msg, "mintupdate-updates-available-symbolic"
                    )
            ...
            if num_visible > 0:
                ...
                if is_self_update:
                    ...
                    self.application.builder.get_object(
                        "label_self_update_details"
                    ).set_text(details)
                else:
                    status_string = gettext.ngettext(
                    ...
                    self.application.builder.get_object("tool_clear").set_sensitive(
                        True
                    )
                    self.application.builder.get_object(
                        "tool_select_all"
                    ).set_sensitive(True)
                    self.application.builder.get_object("tool_apply").set_sensitive(
                        True
                    )
                systray_tooltip = (
                    gettext.ngettext(
                        "%d update available", "%d updates available", num_visible
                    )
                    % num_visible
                )
                self.application.set_status(
                    status_string,
                    systray_tooltip,
                    "mintupdate-updates-available-symbolic",
                    True,
                )
            else:
                ...
                self.application.set_status(
                    ...
                    "mintupdate-up-to-date-symbolic",
                    not self.application.settings.get_boolean("hide-systray"),
                )

            if (
                FLATPAK_SUPPORT
                and self.application.flatpak_updater.error is not None
                and not is_self_update
            ):
                self.application.logger.write(
                    "Could not check for flatpak updates: %s"
                    % self.application.flatpak_updater.error
                )
                msg = (
                    _("Error checking for flatpak updates: %s")
                    % self.application.flatpak_updater.error
                )
            ...
            self.application.logger.write_error(
                "Exception occurred in the refresh thread: %s" % str(sys.exc_info()[0])
            )
            Gdk.threads_enter()
            self.application.set_status(
                _("Could not refresh the list of updates"),
    ...
    def check_policy(self):
        ...
        for line in output.split("\n"):
            ...
            if (
                line.startswith("700")
                and line.endswith("Packages")
                and "/upstream" in line
            ):
    ...
    def mirror_check(self):
        ...
        try:
            if os.path.exists("/usr/bin/mintsources") and os.path.exists(
                "/etc/apt/sources.list.d/official-package-repositories.list"
            ):
                mirror_url = None
                with open(
                    "/etc/apt/sources.list.d/official-package-repositories.list", "r"
                ) as sources_file:
                ...
                if mirror_url is None or not mirror_url.startswith("http"):
                    ...
                    if not self.application.settings.get_boolean("default-repo-is-ok"):
                        ...
                        infobar_message = _(
                            "Local mirrors are usually faster than packages.linuxmint.com."
                        )
                    ...
                    mint_timestamp = self.get_url_last_modified(
                        "http://packages.linuxmint.com/db/version"
                    )
                    mirror_timestamp = self.get_url_last_modified(
                        "%s/db/version" % mirror_url
                    )
                    if mirror_timestamp is None:
                        ...
                        if mint_age > 2:
                            mirror_date = datetime.datetime.fromtimestamp(
                                mirror_timestamp
                            )
                            mirror_age = (mint_date - mirror_date).days
                            if mirror_age > 2:
                                infobar_title = _("Please switch to another mirror")
                                infobar_message = gettext.ngettext(
                                    "The last update on %(mirror)s was %(days)d day ago.",
                                    "The last update on %(mirror)s was %(days)d days ago.",
                                    (now - mirror_date).days,
                                ) % {
                                    "mirror": mirror_url,
                                    "days": (now - mirror_date).days,
                                }
        except:
            print(sys.exc_info()[0])
            # best effort, just print out the error
            print(
                "An exception occurred while checking if the repositories were up to date: %s"
                % sys.exc_info()[0]
            )
...
class Logger:
    ...
    def _create_log(self):
        ...
        self.log = tempfile.NamedTemporaryFile(
            mode="w", prefix=self.logdir, delete=False
        )
        try:
...
class MintUpdate:
    def __init__(self):
        ...
        try:
            ...
            self.textview_packages = self.builder.get_object(
                "textview_packages"
            ).get_buffer()

            self.textview_description = self.builder.get_object(
                "textview_description"
            ).get_buffer()
            self.textview_changes = self.builder.get_object(
                "textview_changes"
            ).get_buffer()
            ...
            self.builder.get_object("button_welcome_finish").connect(
                "clicked", self.on_welcome_page_finished
            )
            self.builder.get_object("button_welcome_help").connect(
            ...
            column_name = Gtk.TreeViewColumn(
                _("Name"), Gtk.CellRendererText(), markup=UPDATE_DISPLAY_NAME
            )
            column_name.set_sort_column_id(UPDATE_DISPLAY_NAME)
            column_name.set_resizable(True)

            column_old_version = Gtk.TreeViewColumn(
                _("Old Version"), Gtk.CellRendererText(), text=UPDATE_OLD_VERSION
            )
            column_old_version.set_sort_column_id(UPDATE_OLD_VERSION)
            column_old_version.set_resizable(True)

            column_new_version = Gtk.TreeViewColumn(
                _("New Version"), Gtk.CellRendererText(), text=UPDATE_NEW_VERSION
            )
            ...
            column_size = Gtk.TreeViewColumn(
                _("Size"), Gtk.CellRendererText(), text=UPDATE_SIZE_STR
            )
            column_size.set_sort_column_id(UPDATE_SIZE)
            column_size.set_resizable(True)

            column_type = Gtk.TreeViewColumn(
                _("Type"), Gtk.CellRendererPixbuf(), icon_name=UPDATE_TYPE_PIX
            )
            column_type.set_sort_column_id(UPDATE_TYPE)
            column_type.set_resizable(True)

            column_origin = Gtk.TreeViewColumn(
                _("Origin"), Gtk.CellRendererText(), text=UPDATE_SOURCE
            )
            ...
            self.builder.get_object("notebook_details").connect(
                "switch-page", self.switch_page
            )
            ...
            self.install_button.add_accelerator(
                "clicked", accel_group, key, mod, Gtk.AccelFlags.VISIBLE
            )
            ...
            select_all_button.add_accelerator(
                "clicked", accel_group, key, mod, Gtk.AccelFlags.VISIBLE
            )
            ...
            refresh_button.add_accelerator(
                "clicked", accel_group, key, mod, Gtk.AccelFlags.VISIBLE
            )

            # Self-update page:
            self.builder.get_object("confirm-self-update").connect(
                "clicked", self.self_update
            )
            ...
            image = Gtk.Image.new_from_icon_name(
                "preferences-other-symbolic", Gtk.IconSize.MENU
            )
            ...
            if os.path.exists("/usr/bin/timeshift-gtk"):
                ...
                sourcesMenuItem = Gtk.ImageMenuItem(
                    label=_("System Snapshots"), image=image
                )
            ...
            if os.path.exists("/usr/bin/mintsources"):
                image = Gtk.Image.new_from_icon_name(
                    "system-software-install-symbolic", Gtk.IconSize.MENU
                )
                sourcesMenuItem = Gtk.ImageMenuItem(
                    label=_("Software Sources"), image=image
                )
            ...
            if os.path.exists(rel_path):
                ...
                if rel_edition.lower() in config["general"]["editions"]:
                    ...
                    image = Gtk.Image.new_from_icon_name(
                        "mintupdate-type-package-symbolic", Gtk.IconSize.MENU
                    )
                    relUpgradeMenuItem = Gtk.ImageMenuItem(
                        label=_("Upgrade to %s") % rel_target, image=image
                    )
            ...
            historyMenuItem = Gtk.ImageMenuItem(
                label=_("History of Updates"), image=image
            )
            ...
            visibleColumnsMenuItem = Gtk.ImageMenuItem(
                label=_("Visible Columns"), image=image
            )
            ...
            typeColumnMenuItem.connect(
                "toggled", self.setVisibleColumn, column_type, "show-type-column"
            )
            visibleColumnsMenu.append(typeColumnMenuItem)

            packageColumnMenuItem = Gtk.CheckMenuItem(label=_("Package"))
            packageColumnMenuItem.set_active(
                self.settings.get_boolean("show-package-column")
            )
            column_name.set_visible(self.settings.get_boolean("show-package-column"))
            packageColumnMenuItem.connect(
                "toggled", self.setVisibleColumn, column_name, "show-package-column"
            )
            visibleColumnsMenu.append(packageColumnMenuItem)

            oldVersionColumnMenuItem = Gtk.CheckMenuItem(label=_("Old Version"))
            oldVersionColumnMenuItem.set_active(
                self.settings.get_boolean("show-old-version-column")
            )
            column_old_version.set_visible(
                self.settings.get_boolean("show-old-version-column")
            )
            oldVersionColumnMenuItem.connect(
                "toggled",
                self.setVisibleColumn,
                column_old_version,
                "show-old-version-column",
            )
            visibleColumnsMenu.append(oldVersionColumnMenuItem)

            newVersionColumnMenuItem = Gtk.CheckMenuItem(label=_("New Version"))
            newVersionColumnMenuItem.set_active(
                self.settings.get_boolean("show-new-version-column")
            )
            column_new_version.set_visible(
                self.settings.get_boolean("show-new-version-column")
            )
            newVersionColumnMenuItem.connect(
                "toggled",
                self.setVisibleColumn,
                column_new_version,
                "show-new-version-column",
            )
            visibleColumnsMenu.append(newVersionColumnMenuItem)

            sizeColumnMenuItem = Gtk.CheckMenuItem(label=_("Origin"))
            sizeColumnMenuItem.set_active(
                self.settings.get_boolean("show-origin-column")
            )
            column_origin.set_visible(self.settings.get_boolean("show-origin-column"))
            sizeColumnMenuItem.connect(
                "toggled", self.setVisibleColumn, column_origin, "show-origin-column"
            )
            ...
            sizeColumnMenuItem.connect(
                "toggled", self.setVisibleColumn, column_size, "show-size-column"
            )
            visibleColumnsMenu.append(sizeColumnMenuItem)
            ...
            descriptionsMenuItem.set_active(
                self.settings.get_boolean("show-descriptions")
            )
            ...
            try:
                ...
                print("Error checking LMDE status:", str(e))
                self.logger.write_error(
                    "Exception occurred while checking LMDE status: " + str(e)
                )
            ...
            self.stack.add_named(
                self.builder.get_object("status_updated"), "status_updated"
            )
            self.stack.add_named(
                self.builder.get_object("status_error"), "status_error"
            )
            self.stack.add_named(
                self.builder.get_object("status_self-update"), "status_self-update"
            )
            self.stack.add_named(
                self.builder.get_object("status_refreshing"), "status_refreshing"
            )
            ...
            self.refresh_schedule_enabled = self.settings.get_boolean(
            ...
            self.logger.write_error("Exception occurred in main thread: " + str(e))
            self.logger.close()
    ...
    def _on_settings_changed(self, settings, key, data=None):
        ...
        self.app_restart_required = (
            settings.get_boolean("show-cinnamon-updates") != self.show_cinnamon_enabled
            or settings.get_boolean("show-flatpak-updates") != self.show_flatpak_enabled
        )
    ...
    def dpkg_locked():
        ...
        try:
            subprocess.run(
                ["sudo", "/usr/lib/linuxmint/mintUpdate/dpkg_lock_check.sh"],
    ...
    def show_infobar(
        self, title, msg, msg_type=Gtk.MessageType.WARNING, icon=None, callback=None
    ):
        infobar = Gtk.InfoBar()
    ...
    def show_welcome_page(self, widget=None):
        ...
        self.set_status(
            _("Welcome to the Update Manager"),
            _("Welcome to the Update Manager"),
            "mintupdate-updates-available-symbolic",
            True,
        )
    ...
    def toggled(self, renderer, path):
        ...
        if iter is not None:
            model.set_value(
                iter, UPDATE_CHECKED, (not model.get_value(iter, UPDATE_CHECKED))
            )
    ...
    def treeview_right_clicked(self, widget, event):
        if event.button == 3:
            ...
            if iter is not None:
                update = model.get_value(iter, UPDATE_OBJ)
                menu = Gtk.Menu()
                menuItem = Gtk.MenuItem.new_with_mnemonic(
                    _("Ignore the current update for this package")
                )
                menuItem.connect(
                    "activate", self.add_to_ignore_list, update.source_packages, True
                )
                menu.append(menuItem)
                menuItem = Gtk.MenuItem.new_with_mnemonic(
                    _("Ignore all future updates for this package")
                )
                menuItem.connect(
                    "activate", self.add_to_ignore_list, update.source_packages, False
                )
                menu.append(menuItem)
    ...
    def open_history(self, widget):
        ...
        column_date = Gtk.TreeViewColumn(
            _("Date"), Gtk.CellRendererText(), text=COL_DATE
        )
        column_date.set_sort_column_id(COL_DATE)
        column_date.set_resizable(True)
        column_type = Gtk.TreeViewColumn(
            _("Type"), Gtk.CellRendererText(), text=COL_TYPE
        )
        column_type.set_sort_column_id(COL_TYPE)
        column_type.set_resizable(True)
        column_package = Gtk.TreeViewColumn(
            _("Update"), Gtk.CellRendererText(), text=COL_NAME
        )
        column_package.set_sort_column_id(COL_NAME)
        column_package.set_resizable(True)
        self.column_old_version = Gtk.TreeViewColumn(
            _("Old Version"),
        ...
        self.column_new_version = Gtk.TreeViewColumn(
            _("New Version"),
            Gtk.CellRendererText(ellipsize=Pango.EllipsizeMode.END),
            text=COL_NEW_VER,
        )
        ...
        if os.path.isfile("/var/log/dpkg.log"):
            apt_updates = (
                subprocess.run(
                    'zgrep " upgrade " -sh /var/log/dpkg.log*',
                    stdout=subprocess.PIPE,
                    stderr=subprocess.DEVNULL,
                    shell=True,
                )
        ...
        def on_query_tooltip(widget, x, y, keyboard, tooltip):
            if not widget.get_tooltip_context(x, y, keyboard):
                ...
                on_row, wx, wy, model, path, iter = widget.get_tooltip_context(
                    x, y, keyboard
                )
    ...
    def open_preferences(self, widget, show_automation=False):
        ...
        stack.add_titled(
            builder.get_object("page_options"), "page_options", _("Options")
        )
        stack.add_titled(
            builder.get_object("page_blacklist"), "page_blacklist", _("Packages")
        )
        ...
        section.add_row(
            GSettingsSwitch(
                _("Hide the update manager after applying updates"),
        ...
        switch = GSettingsSwitch(
            _("Refresh the list of updates automatically"),
            "com.linuxmint.updates",
            "refresh-schedule-enabled",
        )
        ...
        spin_button = GSettingsSpinButton(
            "", "com.linuxmint.updates", "refresh-days", mini=0, maxi=99, step=1, page=2
        )
        ...
        label.set_markup(
            "<i>%s</i>"
            % _(
                "Note: The list only gets refreshed while the Update Manager window is closed (in system tray mode)."
            )
        )
        grid.attach(label, 0, 3, 4, 1)
        section.add_reveal_row(
            grid, "com.linuxmint.updates", "refresh-schedule-enabled"
        )
        ...
        switch = GSettingsSwitch(
            _("Only show notifications for security and kernel updates"),
            "com.linuxmint.updates",
            "tracker-security-only",
        )
        section.add_reveal_row(
            switch, "com.linuxmint.updates", "tracker-disable-notifications", [False]
        )
        switch = GSettingsSpinButton(
            _(
        ...
        switch = GSettingsSpinButton(
            _("Show a notification if an update is older than (in days):"),
            "com.linuxmint.updates",
            "tracker-max-age",
            mini=2,
            maxi=90,
        ...
        if os.path.exists("/usr/bin/cinnamon") or os.path.exists("/usr/bin/flatpak"):
            section = page.add_section(
                _("Update types"), _("In addition to system packages, check for:")
            )

            if os.path.exists("/usr/bin/cinnamon"):
                section.add_row(
                    GSettingsSwitch(
                        _("Cinnamon spice updates"),
                        "com.linuxmint.updates",
        ...
        column = Gtk.TreeViewColumn(
            _("Ignored Updates"), Gtk.CellRendererText(), text=BLACKLIST_PKG_NAME
        )
        ...
        builder.get_object("button_add").connect(
            "clicked", self.add_blacklisted_package, treeview_blacklist, window
        )
        builder.get_object("button_remove").connect(
            "clicked", self.remove_blacklisted_package, treeview_blacklist
        )
        ...
        box.pack_start(page, True, True, 0)
        section = page.add_section(
            _("Package Updates"), _("Performed as root on a daily basis")
        )
        autoupgrade_switch = Switch(_("Apply updates automatically"))
        autoupgrade_switch.content_widget.set_active(
            os.path.isfile(AUTOMATIONS["upgrade"][2])
        )
        autoupgrade_switch.content_widget.connect(
            "notify::active", self.set_auto_upgrade
        )
        ...
        button.set_tooltip_text(
            _("Click this button to make automatic updates use your current blacklist.")
        )
        ...
        if os.path.exists("/usr/bin/cinnamon"):
            switch = GSettingsSwitch(
                _("Update Cinnamon spices automatically"),
                "com.linuxmint.updates",
        ...
        if os.path.exists("/usr/bin/flatpak"):
            switch = GSettingsSwitch(
                _("Update Flatpaks automatically"),
                "com.linuxmint.updates",
        ...
        if len(additional_options) > 0:
            section = page.add_section(
                _("Other Updates"), _("Performed when you log in")
            )
            for switch in additional_options:
                section.add_row(switch)
        section = page.add_section(
            _("Automatic Maintenance"), _("Performed as root on a weekly basis")
        )
        autoremove_switch = Switch(_("Remove obsolete kernels and dependencies"))
        autoremove_switch.content_widget.set_active(
            os.path.isfile(AUTOMATIONS["autoremove"][2])
        )
        autoremove_switch.content_widget.connect("notify::active", self.set_auto_remove)
        section.add_row(autoremove_switch)
        section.add_note(
            _(
                "This option always leaves at least one older kernel installed and never removes manually installed kernels."
            )
        )
    ...
    def export_blacklist(self, widget):
        ...
        with open(filename, "w") as f:
            f.write("\n".join(blacklist) + "\n")
        subprocess.run(
            ["pkexec", "/usr/bin/mintupdate-automation", "blacklist", "enable"]
        )
    ...
    def set_auto_upgrade(self, widget, param):
        ...
        if action:
            subprocess.run(
                ["pkexec", "/usr/bin/mintupdate-automation", "upgrade", action]
            )
    ...
    def set_auto_remove(self, widget, param):
        ...
        if action:
            subprocess.run(
                ["pkexec", "/usr/bin/mintupdate-automation", "autoremove", action]
            )
    ...
    def add_blacklisted_package(self, widget, treeview_blacklist, window):
        ...
        dialog.set_markup(
            _(
                "Please specify the source package name of the update to ignore (wildcards are supported) and optionally the version:"
            )
        )
    ...
    def inhibit_pm(self, reason):
        ...
        try:
            ...
            self.logger.write(
                "Couldn't get session bus to inhibit power management: %s" % e.message
            )
            return
    ...
    def uninhibit_pm(self):
        if self.inhibit_cookie > 0:
            try:
                ...
                self.logger.write(
                    "Couldn't get session bus to uninhibit power management: %s"
                    % e.message
                )
                return

            name, path, iface, unused_args, uninhibit_method = self.get_inhibitor_info(
                "none"
            )
            ...
            try:
                ...
                self.logger.write(
                    "Could not uninhibit power management: %s" % e.message
                )
                return
